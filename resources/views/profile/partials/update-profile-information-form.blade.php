<section>
    <header>
        <h2 class="font-mono text-sm font-semibold text-text-primary uppercase tracking-wider">
            Profile Information
        </h2>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="font-mono text-xs text-text-muted mt-2">
                        ...
                        <button form="send-verification" class="font-mono underline text-neon-cyan hover:text-neon-cyan/80 transition">
                            {{ __('Click here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-mono text-xs text-neon-green">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p
                    ...
                    class="font-mono text-xs text-neon-green"
                >{{ __('Saved.') }}</p>
            @endif
        </div>
    </form>
